...
function TrainerAIConversation() {
  ...
  // Initialize server session and get initial node message
  useEffect(() => {
    (async () => {
      if (!trainer || !trainerId || initialMessageSet.current) return
      try {
        const orgId = (window as any)?.__orgId || (trainer as any).organizationId || ''
        const created = await apiService.createSession({ trainerId, organizationId: orgId })
        const data: any = created?.data
        if (data?.id) setSessionId(data.id)
        const init = data?.initialMessage
        if (init?.content) {
          setMessages([
            {
              id: 1,
        ...
        setMessages([
          {
            id: 1,
            type: 'ai' as const,
            content: `Hello! I'm your ${trainer?.name || 'AI Trainer'}. I'm here to help you with your training session. How can I assist you today?`,
            timestamp: new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })
          }
        ])
        initialMessageSet.current = true
  ...
  const handleSendMessage = async () => {
    ...
    setInputMessage('')
    try {
      const res = await apiService.sendSessionMessage(sessionId, text)
  ...
  return (
    <Layout>
      <Header 
        ...
        subtitle="All aboard! Your training journey is on track"
      />
              ...
              {messages.map((message) => (
                ...
                >
                  <div className={`max-w-[85%] md:max-w-[70%] ${message.type === 'user' ? 'bg-[#40B1DF] text-white' : 'bg-[#f8fafc] text-[#313F4E]'} rounded-2xl px-3 sm:px-4 py-2 sm:py-3 font-family: Inter, sans-serif transition-all duration-200 ease-in-out`}>
                    {message.mediaUrl && message.type === 'ai' ? (
                      <div className="mb-2">
                        <img src={message.mediaUrl} alt="attachment" className="rounded-lg max-h-64 object-contain" />
                      </div>
                    ) : null}
      ...
      >
        <div className="space-y-3">
          <p className="text-sm text-[#64748b]">
            Welcome aboard! This is your training session with {trainer?.name || 'AI Trainer'}. Let's propel your learning forward:
          </p>
